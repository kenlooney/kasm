...
Kasm 0.9.0 (in development) loads assembly source, parses statements, label
definitions, and nested blocks, validates
operands, evaluates expressions, and assigns byte offsets before encoding.
It encodes `mov eax, <expression>;`, `ret;`, and `jmp <label>;`
...
Version **0.5.0 is released**. Versions **0.6.0 through 0.9.0 are unreleased
development milestones**; the current source version is 0.9.0. The descriptions
below track completed features even when a milestone has no separate release.
...
| 0.6.0 (unreleased) | Add Windows x86-64 execution to the runner using the same generated bytes. |
| 0.7.0 (unreleased) | Parse label definitions, including consecutive labels, without emitting extra bytes. |
| 0.8.0 (unreleased) | Assign instruction and label offsets before encoding, reject duplicate labels, and provide label-offset lookup. |
| 0.9.0 (unreleased) | Encode near jumps with signed relative displacements and resolve forward label references after layout. |
...
the operand-free instruction `ret;`, and `jmp <identifier>;`, as well as
...
or a near JMP, one for RET, and zero for a label. Offsets are relative to the beginning of the
...
`jmp <label>;` encodes an unconditional near jump as `E9` followed by a signed
...
The current language supports MOV to `eax`, operand-free RET, and near JMP to
a label. Short jumps, conditional jumps, other instructions and registers,
...
The label and near-jump samples, C inspection, and Linux/Windows execution examples above are manual checks; they
