...
Kasm 0.34.0 (in development) is a small C assembler with a limited
...
The current source version is **0.34.0 (in development)**.

The full version-by-version history — from the first lexer commit through
0.33.0's COFF object output and the 0.34.0 journal launch — is now kept as a living journal in
[`devlog/`](devlog/src/introduction.md), built with mdbook. Read it for the
story of how each capability was added and why; this README stays focused on
current behavior and usage.

To build the journal locally:
...
## Boot-sector image fixture (0.28.0)

The 16-bit boot-sector example combines instruction encoding, location-aware
padding, and binary literal parsing:
