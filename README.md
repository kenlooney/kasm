...
Kasm 0.7.0 (in development) loads assembly source, parses statements, label
definitions, and nested blocks, validates
operands, evaluates expressions, and assigns byte offsets before encoding.
It encodes `mov eax, <expression>;` and
`ret;` as x86 machine-code bytes, prints hexadecimal output, and writes both a
...
Version **0.5.0 is released**. Versions **0.6.0 and 0.7.0 are unreleased
development milestones**; the current source version is 0.7.0. The descriptions
...
| Next milestone (unreleased; version pending) | Assign instruction and label offsets before encoding, reject duplicate labels, and provide label-offset lookup. |
...
and the operand-free instruction `ret;`, as well as `identifier:` label definitions.
...
Instruction names are case-sensitive: use lowercase `mov` and `ret`.
...
names are rejected. Using labels as instruction operands or expression values,
and jumping to labels, are not implemented yet.
...
one for RET, and zero for a label. Offsets are relative to the beginning of the
...
For this example, `entry` resolves to 0 and `done` to 5. This lookup API prepares
for future label references; the source grammar does not accept them yet.
...
The current language supports MOV to `eax` and operand-free RET. Other instructions and
registers, label references in instructions or expressions, memory operands, directives, and object
...
definition, for example `label:`. Label references are not supported yet.
...
The label samples, C inspection, and Linux/Windows execution examples above are manual checks; they
